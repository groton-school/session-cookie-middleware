...
class Session implements SessionInterface, SessionManagerInterface
{
    ...
    {
        throw new SessionException('Session must be started by middleware');
    }

    public function middlewareStart(RequestInterface $request)
    {
        if ($this->isStarted()) {
        ...
        $sessionId = $this->options['id'] ?? FigRequestCookies::get($request, $this->options['name']);
        if ($sessionId) {
